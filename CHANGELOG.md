...
- 动机：T-025 收尾时用户发现两个既有缺陷：①L2 dev/close 阶段实际由 Main Agent 主线程承担，不派发 `sage_coder`/`sage_closer` 执行型子代理——`orchestrator.md` L2 行「可子代理化」与 `execution-channels.md` 默认通道矩阵「coder/closer → subagent」冲突，T-025 拿「可」字当借口；`omp.md` 限制段登记「此为编排层既有行为（T-008 至今未改）」。②计划放行请求以 `ask` 弹窗弹出后「很快就过去了」——`ask` schema 无 timeout（`additionalProperties: false`，跨 IDE 行为不一致），turn 内无「未放行即停」强制机制，五要素原文未落盘 TASK 事后不可追溯。
- 修复内容：①`core/prompts/orchestrator.md` L2 行「可子代理化」→「必须子代理化（原生通道可用时）」与 execution-channels 矩阵对齐；「约束与红线」新增「未放行 turn 级强制停止」铁律（每 turn 入口先读 TASK 元数据「计划放行」，非「已放行」则仅输出等待并 yield，不调用工具/不写文件/不派发；L0/L3 豁免）；阶段流转图放行门注脚「放行前禁止主代理代行执行角色」。②`core/prompts/planner.md` 第 8 节新增「落盘要求」——放行请求五要素原文必须写入 TASK 持久化（L2→2.x 节固定小节「计划放行请求」、L1→1.5 后固定小节同名），不依赖 ask 弹窗；禁令补「未获用户明确「放行」或回应时，停止并 yield，禁止推进至 dev」。③`core/scripts/sage_linter.py` 新增第 19 个检查器 `check_clearance_request_record`（SAGE-19，编号 [19/19]，rule_id 显式映射）——判定矩阵（L0/L3 跳过、init/plan-review 未到期跳过、dev/code-review/close 到期强制、未知阶段/风险缺失 fail-safe），L2 检查 2.x、L1 检查 1.5 后同名小节（正则 `1\.(?:[6-9]|\d{2,})` 严格对齐「1.5 后」），五要素关键词容错正则（可写.{0,4}清单 / 自决.{0,4}清单 / 风险.{0,8}概览，兼容「可写文件清单/自决项清单/风险与缓解概览」）。④配套单测 `CheckClearanceRequestRecordTests` 10 例（L2 dev 通过/缺关键词阻断/缺小节阻断、init·plan-review 跳过、L0·L3 豁免、L1 1.6 落盘通过、L1 1.3 误拒、风险缺失 fail-safe、code-review·close 到期态、未知阶段 fail-safe）；全量 111 例 OK（101 基线 + 10 新增，零回归）。⑤`adapters/omp/omp.md` 限制段改「四阶段均派发，全部 modelRoles 键生效」（删除「预留位/当前不消费」）。⑥`core/guides/execution-channels.md` 矩阵注脚「L2 主代理不得代行执行角色」+ 补 L1 可选口径消除歧义；`CODE_WIKI.md` 4.1 检查器清单补第 19 项 SAGE-19 + 数量 17/18→19。⑦`.gitignore` 追加 `.omp/`（provision 产物不入库，T-025 遗留债一次性修复）。
- 本任务自身试点：dev 阶段真实派发 `sage_coder` 子代理（agent_type=sage_coder，modelRoles.sage_coder，model=litellm/deepseek-v4-flash）执行 1.1~1.5 全部实现，`dispatch_phase.py verify` 实证 subagent 实际执行并修改 TASK/Git；close 阶段主线程承担（sage_closer 键已就绪可消费，子代理试点顺延至下个 L2 任务）。
- 关键决策（详见 DECISION_LOG）：编排契约束缚（orchestrator 对齐矩阵，消除「可」字冲突）；放行门三通道交付（ask 弹窗 + 正文文本 + TASK 落盘，SAGE-19 检查器）；跨 IDE 不做 turn 级硬门禁（宿主 auto-continue 行为不一致，用提示词铁律 + linter 阶段检查 SAGE-18/19 组合）；dispatch_phase.py 零改动（T-025 先例，dispatcher 本身支持 dev/close 派发）。

## [1.8.4] ✨ Feature OMP/codex provision 统一：OMP 审查合并三角色对齐 codex，双目录部署（项目默认 + --user 用户注册目录） (T-025) - 2026-09-02 11:00:00
- 动机：用户在 OMP 实测 + 复核 codex 适配器后提出两点：①OMP 审查阶段（plan-review+code-review）应像 codex 那样共用一个 `sage_reviewer`（codex 本就是三角色 sage_reviewer/sage_coder/sage_closer），每阶段独立 4 agent 命名跨适配器心智分裂；②provision 需支持部署到宿主用户注册目录（新增 `--user`），默认仍是项目目录，omp.md/codex.md 写清复制方式给 AI 读实现「一句话自动部署」。
- 修复内容：①`omp.json` agent_types 从 4 独立 agent 改为三角色（plan-review→sage_reviewer、dev→sage_coder、code-review→sage_reviewer、close→sage_closer），`models.<phase>.id` 保持实际模型标识符不变；`omp/provision.py` 重构为按角色生成 3 agent（sage_reviewer/sage_coder/sage_closer.md）+ config.yml 3 个 modelRoles 键 + advisor 固定键，reviewer 模型取 plan-review 首个非空、code-review 不一致时告警（对齐 codex build_toml_agent 语义）。②`--target-dir` 改可选默认项目目录（OMP `<cwd>/.omp`、codex `<cwd>/.codex/agents`），新增 `--user` 部署宿主用户注册目录（OMP `~/.omp/agent`、codex `~/.codex/agents`），与 `--target-dir` 互斥报错；codex `--target-dir` 从 required 改可选。③`omp.md`/`codex.md` 重写：三角色映射表 + 双目录部署 + 一句话自动部署说明；修复 codex.md 既有缺陷 F13（模型值陈旧 claude-opus-4-7→gpt-5.6-sol/terra/luna、命名下划线/连字符混用统一）；`model-selection.md`/`execution-channels.md` OMP 行同步三角色（清除 T-023 旧设计残留 sage-slow/sage-task）。④`test_dispatch_phase.py`：OMP 3 例重写为三角色断言 + 新增 OMP `--user`/codex 默认目录/`--user`/互斥报错测试。
- 关键决策（详见 DECISION_LOG）：审查合并三角色（plan-review+code-review 共用 sage_reviewer 键，跨适配器命名统一）；双目录部署（默认项目 + `--user` 用户注册目录）；dispatch_phase.py 零改动（--user 仅适配器脚本参数，主入口 --target-dir 仍 required）。
- 验证：全量单测 101 例 OK 零回归（99 基线 + 2 新增）；独立实跑 OMP/codex provision（默认目录、--user、互斥报错、--role 过滤、增量合并）通过；计划盲审 WARN 无阻塞（5 条建议全落实）+ 代码盲审 WARN 无阻塞（3 条建议 W1 文件名统一/W2 告警收窄/W3 常量提取全落实）。
...
- provision 易用性改进（默认目录 + 增量合并）：`--target-dir` 从 required 改为可选，默认 `<当前目录>/.omp`，不带参数即可部署；config.yml 写入语义改为三态——不存在→新建；已存在+不带 `--force`→**增量合并**（保留非 modelRoles 段如 settings/task，只替换 modelRoles 段）；已存在+`--force`→整文件覆盖（有告警）。新增 `merge_config_yml` 函数逐行扫描保留其他段；agent 文件维持幂等（存在→skipped/`--force`→覆盖）。全量单测 99 例 OK；实跑验证增量合并保留 settings+task 段，modelRoles 段替换为 4 阶段独立键。
